...
- **В GitHub Releases нет собранного APK** — сознательное решение: ключ NewsAPI встраивается в `BuildConfig` как обычная строка и легко достаётся из публичного APK декомпиляцией, а поднимать прокси-бэкенд ради демо-релиза избыточно. Проект собирается локально по инструкции выше.
